...
//            println("view.capacity: ${view.capacity}")
    ...
    fun onItemDropSingleClick(event: ClickInventoryEvent.Drop.Single, @First player: Player) {
        val gui = plugin.guistory.currentGui(player, event.targetInventory) ?: return
...
//        }
    ...
    fun onLeftClick(event: ClickInventoryEvent.Primary, @First player: Player) {
        val gui = plugin.guistory.currentGui(player, event.targetInventory) ?: return
        val view = gui.getViewOf(player)
        forEachItem(view, event) { item, index ->
            val e = ItemClickEvent.Left(event, player, gui, view, item, index)
//            val slotInv = event.targetInventory.query<Inventory>(QueryOperationTypes.INVENTORY_PROPERTY.of(SlotIndex.of(index))).first<Inventory>()
            e
        }
    }
