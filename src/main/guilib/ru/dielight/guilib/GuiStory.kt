...
class GuiStory {
    ...
    fun openPrev(player: Player) {
        val story = guiStory[player.uniqueId] ?: return
        val iterator = story.queue.descendingIterator()
        if (iterator.hasNext()) {
            iterator.next()
            if (iterator.hasNext()) {
                val gui = iterator.next()
                open(player, gui)
                story.opened = true
                return
            }
    ...
    fun currentGui(player: Player, inv: Inventory): Gui? {
        ...
        for (i in 0 until toRemove) {
            story.queue.removeLast().onClose(player)
        }
        if (DEBUG) Sponge.getServer().broadcastChannel.send(
            Text.of(
                "current(" + toRemove + "): " + toString(story.queue.peekLast()) + " " + toString(
                    player
                )
            )
        )
