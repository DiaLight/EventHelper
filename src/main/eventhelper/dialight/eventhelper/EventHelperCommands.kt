...
fun EventHelperPlugin.registerCommands() {
...
class ItemCommand(val plugin: EventHelperPlugin) : CommandCallable_stub() {

    override fun process(src: CommandSource, arguments: String): CommandResult {
        if (src is Player) {
            plugin.toollib.giveTool(src, EventHelperTool.ID)
        }
        return CommandResult.success()
    }
...
class GuiCommand(val plugin: EventHelperPlugin) : CommandCallable_stub() {
...
class EventHelperCommand(val plugin: EventHelperPlugin) : CommandExecutor {
