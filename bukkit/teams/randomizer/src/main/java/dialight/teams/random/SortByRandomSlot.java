...
public class SortByRandomSlot extends DynamicSlot {

    private final SortByRandom proj;
    private final ObservableScoreboard scoreboard;

    public SortByRandomSlot(SortByRandom proj) {
        this.proj = proj;
        this.scoreboard = proj.getTeams().getScoreboardManager().getMainScoreboard();
        ObservableSet<String> teamFilter = proj.getTeams().getTeamWhiteList();
        teamFilter.onAdd(this, this::update);
        teamFilter.onRemove(this, this::update);
        ObservableSet<UUID> playerFilter = proj.getTeams().getPlayerBlackList();
