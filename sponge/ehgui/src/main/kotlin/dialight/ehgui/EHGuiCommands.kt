...
fun EHGuiPlugin.registerCommands() {
...
class ItemCommand(val plugin: EHGuiPlugin) : CommandCallable_stub() {

    override fun process(src: CommandSource, arguments: String): CommandResult {
        if (src is Player) {
            plugin.toollib.giveTool(src, EHGuiTool.ID)
        }
        return CommandResult.success()
    }
...
class GuiCommand(val plugin: EHGuiPlugin) : CommandCallable_stub() {
...
class EventHelperCommand(val plugin: EHGuiPlugin) : CommandExecutor {
