...
class EHSnapshot(val plugin: EHGuiPlugin, id: Identifiable) : Snapshot<Snapshot.Page>(plugin.guilib, id) {
...
//                "|g|Версия: |y|v" + plugin.container.version.orElse("null")
    ...
    fun updateItems() {
        ...
        val toolItems = plugin.toollib.toolregistry.entries.stream()
            ...
            .toList()
        val rawToolPages = EHPageBuilder(maxColumns - 2, maxLines, toolItems).toList()
        ...
        val moduleItems = plugin.modulelib.moduleregistry.entries.stream()
            ...
            .toList()
        val rawModulePages = EHPageBuilder(maxColumns - 2, maxLines, moduleItems).toList()
