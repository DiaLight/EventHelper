...
class MainSnapshot(val plugin: MainGuiPlugin, id: Identifiable) : Snapshot<Snapshot.Page>(plugin.guilib, id) {
...
//                "|g|Версия: |y|v" + plugin.container.version.orElse("null")
    ...
    fun updateItems() {
        ...
        val toolItems = plugin.toollib.toolregistry.entries.stream()
            ...
            .toList()
        val rawToolPages = MainPageBuilder(maxColumns - 2, maxLines, toolItems).toList()
        ...
        val moduleItems = plugin.modulelib.moduleregistry.entries.stream()
            ...
            .toList()
        val rawModulePages = MainPageBuilder(maxColumns - 2, maxLines, moduleItems).toList()
